...
export async function connectRabbitMQ(attempts: number = 5, delayMs: number = 3000): Promise<amqp.Connection> {
    for (let i = 0; i < attempts; i++) {
        try {
            const connectionString = "amqp://rabbitmq:5672";
            return await amqp.connect(connectionString);
        } catch (error) {
            console.log(`Failed to connect to RabbitMQ. Retrying in ${delayMs} ms...`);
